function [doa_meters, doa_samples, doa_meters_2, doa_samples_2, correlation_value, correlation_value_interp] = ...
    tdoa_zurich_2(signal1_complex, signal2_complex, num_samples_per_freq, num_samples_per_slice, sample_rate, rx_distance_diff, ...
    max_lag, corr_type,  report_level, signal_bandwidth_khz_rs, signal_bandwidth_khz_us, interpol, s1, s2)
...
[peaks, locations] = findpeaks(corr_normalized/max(abs(corr_signal_2)),'MinPeakHeight',0.3,'MinPeakProminence',0.1);
...
elseif s1 == "dshush_2" && s2 == "ETHZ_ETZ_Dach"
    loc = min(locations);
    lag = lags_normalized(loc);
elseif s1 == "alemino_ZRH" && s2 == "ETHZ_ETZ_Dach"
...
delay2_native = lag;
idx2 = find(lags2 == lag);
correlation_value(2) = corr_signal_2(idx2);
...
if (interpol > 1)
    ...
    idx2_i = find(lags2_interp == lag);

    lags2_i = lags2_interp(idx2_i-interpol-1:idx2_i+interpol+1);
    [~,idx2_interp] = max(corr_signal_2_interp(idx2_i-interpol-1:idx2_i+interpol+1));
    
    correlation_value_interp(2) = corr_signal_2_interp(idx2_interp);
    delay2_interp = lags2_i(idx2_interp);
else
    delay2_interp = 0;
end
