                ...
                for kk = 1:length(multipath)
                    ...
                    for foldernum = 1:length(folders)
                        ...
                        for filenum = 0:files(foldernum)-1
                            ...
                            res = tdoa_localization_no_ref(problem_data);
                    ...
                    if multipath(kk)
                        ...
                        filename = sprintf("sensys/madrid/s%i/m%s_i%i_c%i_l%i_mp_no_ref.mat", NUM_SENSORS, corr_methods(corr_id), interp(interp_id), correct(ii), lte(jj));
                        save(filename,'lls', 'error_lls', 'nlls', 'error_nlls');
                    else
                        % Saving our beloved file
                        filename = sprintf("sensys/madrid/s%i/m%s_i%i_c%i_l%i_no_ref.mat", NUM_SENSORS, corr_methods(corr_id), interp(interp_id), correct(ii), lte(jj));
                        save(filename,'lls', 'error_lls', 'nlls', 'error_nlls');
                    end
                end
            end % lte
