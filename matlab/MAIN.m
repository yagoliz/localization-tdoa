...
if step_2 == 1
    ...
    for index1=1:N_receiver
        for index2 = 1:Test_for_y1y2
            ...
            index_samples = [0.2e6 0.8e6 1.2e6 1.8e6 2.2e6 2.8e6];
...
% % % % % % %             r2_filt_B = interp(r2_filt_B,4);
            ...
            corr_signal = correlate_iq(r1_filt_B(index_samples(1):index_samples(2)), r2_filt_B(index_samples(1):index_samples(2)), method, 0);
            ...
            % second corr
            corr_signal = correlate_iq(r1_alignment(index_samples(3):index_samples(4)), r2_alignment((index_samples(3):index_samples(4))), method, 0);
            [~, idx1] = max(corr_signal);
            delay_time_US(index1,index2) = idx1 - (index_samples(4)-index_samples(3));
            % third corr
            corr_signal = correlate_iq(r1_alignment(index_samples(5):min([index_samples(6) numel(r1_alignment) numel(r2_alignment)])), r2_alignment((index_samples(5):min([index_samples(6) numel(r1_alignment) numel(r2_alignment)]))), method, 0);
            [~, idx1] = max(corr_signal);
...
if saveall
    save('delay_time_RS','delay_time_RS')
    save('delay_time_US','delay_time_US')
    save('delay_time_RS_ceck','delay_time_RS_ceck')
end
