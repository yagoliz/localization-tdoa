...
% load('sensors');
error_array = zeros(18-2,20);
for jj = 1:20
    sensor = [xmax*rand(20,1), ymax*rand(20,1)];
    ...
    %     [x, y] = find(mse_doa == max(mse_doa(:)));
        [x, y] = solution2d(doa_array, sensor(1:NUM_SENSORS,:));
        max_error = inf;
        if length(x) > 1
            
        else
            max_error = norm(transmitter - [x(1)*r/xmax,y(1)*r/ymax]);
        end
        error_array(NUM_SENSORS-2,jj) = max_error;
