...
doa_samples = zeros(length(filesdev1_tdoa)*3,length(num_samples_per_slice));
doa_meters  = zeros(length(filesdev1_tdoa)*3,length(num_samples_per_slice));
doa_samples_2 = zeros(length(filesdev1_tdoa)*3,length(num_samples_per_slice));
doa_meters_2 = zeros(length(filesdev1_tdoa)*3,length(num_samples_per_slice));
correlation_value = zeros(3,3,length(filesdev1_tdoa)*3,length(num_samples_per_slice));
correlation_value_interp = zeros(3,3,length(filesdev1_tdoa)*3,length(num_samples_per_slice));
...
for filenum=1:length(filesdev1_tdoa)
    ...
    for jj = 1:length(num_samples_per_slice)
        disp(' ');
        disp('______________________________________________________________________________________________');
        disp('CORRELATION 1 & 2');
        [doa_meters(filenum*3-2,jj), doa_samples(filenum*3-2,jj), doa_meters_2(filenum*3-2,jj), doa_samples_2(filenum*3-2,jj), correlation_value(1,:,filenum,jj), correlation_value_interp(1,:,filenum,jj)] = ...
            tdoa2(signal1, signal2, num_samples_per_freq, num_samples_per_slice(jj), sampling_rate_tdoa, rx_distance_diff12, rx_distance12, smoothing_factor, corr_type, report_level, signal_bandwidth_khz, interpol_factor);

        disp(' ');
        disp('______________________________________________________________________________________________');
        disp('CORRELATION 1 & 3');
        [doa_meters(filenum*3-1,jj), doa_samples(filenum*3-1,jj), doa_meters_2(filenum*3-1,jj), doa_samples_2(filenum*3-1,jj), correlation_value(2,:,filenum,jj), correlation_value_interp(2,:,filenum,jj)] = ...
            tdoa2(signal1, signal3, num_samples_per_freq, num_samples_per_slice(jj), sampling_rate_tdoa, rx_distance_diff13, rx_distance13, smoothing_factor, corr_type, report_level, signal_bandwidth_khz, interpol_factor);

        disp(' ');
        disp('______________________________________________________________________________________________');
        disp('CORRELATION 2 & 3');
        [doa_meters(filenum*3,jj), doa_samples(filenum*3,jj), doa_meters_2(filenum*3,jj), doa_samples_2(filenum*3,jj), correlation_value(3,:,filenum,jj), correlation_value_interp(3,:,filenum,jj)] = ...
            tdoa2(signal2, signal3, num_samples_per_freq, num_samples_per_slice(jj), sampling_rate_tdoa, rx_distance_diff23, rx_distance23, smoothing_factor, corr_type, report_level, signal_bandwidth_khz, interpol_factor);
    end
end
