...
file_fo_interp5 = paste(folder, paste("fo_correction_05_interp", paste(type, ".mat", sep=""), sep="_"), sep="")
...
doa_meters_fo_interp5 = abs(data_fo_interp5$doa.meters) + resolution / 5
doa_meters_fo_interp20 = abs(data_fo_interp20$doa.meters) + resolution / 20
doa_meters_fo_interp50 = abs(data_fo_interp50$doa.meters) + resolution / 50
...
labels = c(rep("Panoradio", length(doa_meters_fo)), rep("FOC", length(doa_meters_fo)), rep("FOC + Int (5)", length(doa_meters_fo_interp5)), rep("FOC + Int (20)", length(doa_meters_fo_interp20)), rep("FOC + Int (50)", length(doa_meters_fo_interp50)))
...
p <- p + xlab("Method") + ylab("TDOA (m)") + theme(legend.position="none") + ylim(0, 750) # + ggtitle(paste("Results with", type, sep=" "))
